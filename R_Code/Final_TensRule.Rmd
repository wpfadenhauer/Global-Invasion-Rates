...
Average range sizes (reported as the number of L3 regions included).
```{r}
#Size of average invaded range
#avg_i <- separate_rows(data, Invasive_L3, sep=",")
#avg_i <- avg_i[!(avg_i$Invasive_L3==""), ]
#avg_i$point <- as.numeric(paste("1"))
#avg_i <- avg_i %>%
...
#Size of average established range
#avg_e <- separate_rows(data, Established_L3, sep=",")
#avg_e <- avg_e[!(avg_e$Established_L3==""), ]
#avg_e$point <- as.numeric(paste("1"))
#avg_e <- avg_e %>%
#    group_by(Accepted_name, status) %>% 
#    summarise(sum = sum(point))

#i_m <- avg_e[avg_e$status=="invasive",]
#e_m <- avg_e[avg_e$status=="established",]
...
median(AllContsDestinations$InvasionRate)
hist(AllContsDestinations$InvasionRate)
...
median(All2Destinations$InvasionRate)
hist(All2Destinations$InvasionRate)
...
median(All3Destinations$InvasionRate)
hist(All3Destinations$InvasionRate)
...
median(All4Destinations$InvasionRate)
hist(All4Destinations$InvasionRate)
...
```{r}
#Export plot
tiff("C:/Users/wpfadenhauer/OneDrive - University of Massachusetts/CH2_InvaderFlows/Figures and Figure Data/TensRule/test.tiff",
     units = "in",
     width = 1.2671,
     height = 3.321,
     res = 800)
...
#Boxplot of invasion rate by scale. 
boxplot(InvasionRate~Scale, data = pd)
 abline(h=0.1, col="red", lwd=2)
#Here, the medians look closer to 10%, but the means we calculated/plotted above seem to be better visualizations of the significance testing results.
#This is why Table 1 is important - it breaks down these invasion rate by island/mainland, scale, and climate, which gives a much better view of the variation at each scale. See Table1.Rmd (in the same GitHub repo) for that data. 
```
